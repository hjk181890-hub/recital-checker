...
2. 표지: 파트(Soprano/Mezzo-Soprano/Tenor/Baritone/Bass-Baritone/Bass/Countertenor) 이름 앞 표기 필수, 지도교수 표기 필수(전임교수명 교수/지도교수명 교수 형식 또는 괄호형식 모두 허용, 지도교수만 있는 경우도 허용), Acc.반주자명 필수, 날짜·시간·장소 필수. 학기 표기는 반드시 '박사과정 N학기' 형식이어야 하며 '박사과정 학기 N' 형식은 오류
...
9. 종류별조건: 표준=45분이상50분미만, 박사1학기=30분미만(29분59초이하 포함)+3시대+3언어+연가곡필수, 렉쳐=Lecture Recital표기+해설부·공연부 분리+연주부분 30분미만(29분59초이하)+해설+연주 합산 45-50분
10. 과정문구: 석사='본 연주는 국립창원대학교 일반대학원 석사과정 이수를 위한 필수 연주 과정입니다. This recital is a required concert in fulfillment part of MM program.' 문구 필수, 박사='본 연주는 국립창원대학교 일반대학원 박사과정 이수를 위한 필수 연주 과정입니다. This recital is a required concert in fulfillment part of DMA program.' 문구 필수
11. 페이지: 1페이지 초과시 오류
12. 글꼴: PDF에서 폰트 확인 불가이므로 항상 주의 안내로 표시
13. 파일명: 형식은 반드시 '과정 이름 연도-학기 전공 학위 리사이틀 프로그램' (공백 구분, 대괄호 없음). 예시: 박사 청하오난 2026-1 성악 학위 리사이틀 프로그램
...
16. 작품번호·카탈로그 번호 정확성: op., BWV, HWV, K., D., Hob., RV 등 번호가 실제 작품과 일치하는지 확인. 오류 발견 시 정확한 번호 제시
17. 아리아·오페라 일치 여부: 표기된 아리아가 실제로 해당 오페라에 수록된 곡인지 확인. 오류 발견 시 정확한 정보 제시
18. 원어 스펠링 정확성: 작곡가명, 곡명, 오페라명, 연가곡명 등 모든 원어 텍스트의 스펠링을 정밀 검토. 악센트 부호·움라우트·특수문자 포함. 단, 독일어 구철자법(예: wüßten's 등 악보 원전 표기)은 오류로 처리하지 말 것. 오류 발견 시 정확한 스펠링 제시
19. 연가곡 곡번호·소제목 일치: 연가곡의 각 곡 번호와 소제목이 실제 악보 순서 및 제목과 일치하는지 확인. 예: Winterreise Nr.1=Gute Nacht, Nr.5=Der Lindenbaum, Nr.24=Der Leiermann. Dichterliebe Nr.1=Im wunderschönen Monat Mai. 오류 발견 시 정확한 정보 제시
